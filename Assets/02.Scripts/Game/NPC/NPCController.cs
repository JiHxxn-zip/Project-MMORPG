...
namespace MMORPG.Game
{
    public class NPCController : MonoBehaviour
    {
        ...
        /// 우선순위: TalkToNPC 목표 > 퀘스트 진행 중(Active 대사) > 퀘스트 수락 > 기본 대화
        /// 퀘스트 완료는 EventBus가 조건 충족 시점에 자동 처리하므로 NPC 반납 분기 없음.
        /// </summary>
        public QuestContext GetInteractionContext()
        {
            if (QuestManager.Instance != null)
            {
                // 이 NPC가 TalkToNPC 퀘스트의 목표인 경우 — 대화 후 NpcTalked 발행 → 자동 완료
